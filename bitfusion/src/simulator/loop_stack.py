...
class LoopStack(object):
    ...
    def insert_loop(self, loop_count, stride, level=0, name=None):
        ...
        loop = LoopInstruction(
            name=name, loop_count=loop_count, stride=stride, level=level
        )
    ...
    def promote_mem_ops(self, sram):
        for op in self.mem_ops:
            ...
            while curr is not None and (
                curr.stride[namespace] == 0
                or sram[namespace] > op.size * curr.loop_count
            ):
    ...
    def get_stats(self, acc_obj, verbose=False):
        ...
        for l in self.loop_stack:
            if l is not None and l.level == 0 and isinstance(l, LoopInstruction):
                stats[l.name] = l.get_stats(
                    acc_obj, self.mem_ops, self.compute_ops, top=True
                )
...
class MemoryReadInstruction(Instruction):
    ...
    def __str__(self, l=0):
        ret = " | " * l + ("{0}: addr {1}, size {2}, stride {3}").format(
            self.name, self.base_addr, self.size, self.stride
        )
...
class LoopInstruction(Instruction):
    ...
    def __str__(self, l=0):
        ret = " | " * l + "{0}: Range {1}, stride {2}".format(
            self.name, self.loop_count, self.stride
        )
    ...
    def set_loop_with_level(self, level, loop):
        ...
        if level == self.level + 1:
            ...
            if len(self.inner_loop) == 0:
                raise IndexError(
                    "Inserting loop {0} with level {1} into loop {2} with level {3}".format(
                        loop.name, loop.level, self.name, self.level
                    )
                )
    ...
    def get_pipe_stats(self, acc_obj, mem_ops, compute_ops, top=False):
        ...
        for il in self.inner_loop:
            if isinstance(il, MemoryReadInstruction):
                rd_stats += (
                    il.get_stats(acc_obj, mem_ops, compute_ops) * self.loop_count
                )
        ...
        if il_count == 0:
            ...
            pipeline = Pipeline(
                compute_stats.total_cycles,
                rd_stats.total_cycles // self.loop_count,
                wr_stats.total_cycles // self.loop_count,
                self.loop_count,
            )
            total_stats = rd_stats + compute_stats * self.loop_count + wr_stats
            total_stats.total_cycles = pipeline.get_cycles()
            total_stats.mem_stall_cycles = (
                total_stats.total_cycles - compute_stats.total_cycles * self.loop_count
            )
        else:
            for il in self.inner_loop:
                count = 0
                if isinstance(il, LoopInstruction):
                    ...
                    total_stats.mem_stall_cycles = (
                        ...
                        - (il_stats.total_cycles - il_stats.mem_stall_cycles)
                        * self.loop_count
                    )
