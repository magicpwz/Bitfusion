...
def conv(
    ...
    weights = get_tensor(
        shape=(filters, kernel_size, kernel_size, input_channels),
        name="weights",
        dtype=w_dtype,
    )
    ...
    # 可以把tensor_in拆开来进行不同的卷积操作
    # or重写conv2D
    # 更细粒度的动态
    # 可以试着把之前dice的动态网络放进去跑
...
def get_bench_numbers(graph, sim_obj, batch_size=1):
    ...
    for opname, op in graph.op_registry.items():
        # # conv1_a/Convolution
        # print("opname", opname)
        # # <dnnweaver2.tensorOps.cnn.Convolution object at 0x7efdba06fe50>
        # print("op", op)
        # sys.exit()
...
def get_alex_net():
    ...
    with g.as_default():
        ...
        with g.name_scope("pool1_a"):
            pool1_a = maxPool(
                conv1_a, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("pool1_b"):
            pool1_b = maxPool(
                conv1_b, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
...
def get_alex_net_wrpn():
    ...
    with g.as_default():
        ...
        with g.name_scope("fc2"):
            fc2 = fc(
                fc1, output_channels=8192, w_dtype=FQDtype.FXP4, f_dtype=FQDtype.FXP8
            )
...
def get_RNN(name, size):
    ...
    with g.as_default():
        with g.name_scope("inputs"):
            i = get_tensor(
                shape=(batch_size, size * 2),
                name="data",
                dtype=FQDtype.FXP4,
                trainable=False,
            )
        with g.name_scope("matmul"):
            out = fc(i, output_channels=2 * size, w_dtype=FQDtype.FXP4, f_dtype=None)
    return g
...
def get_LSTM(name, size):
    ...
    with g.as_default():
        with g.name_scope("inputs"):
            i = get_tensor(
                shape=(batch_size, size * 4),
                name="data",
                dtype=FQDtype.FXP4,
                trainable=False,
            )
        with g.name_scope("matmul"):
            out = fc(i, output_channels=4 * size, w_dtype=FQDtype.FXP4, f_dtype=None)
    return g
...
def get_svhn_qnn():
    ...
    with g.as_default():
        ...
        with g.name_scope("pool1"):
            pool1 = maxPool(
                conv1, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("pool3"):
            pool3 = maxPool(
                conv3, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("pool5"):
            pool5 = maxPool(
                conv5, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("fc6"):
            fc6 = fc(
                flatten5, output_channels=1024, w_dtype=FQDtype.Bin, f_dtype=FQDtype.Bin
            )

        with g.name_scope("fc7"):
            fc7 = fc(
                fc6, output_channels=1024, w_dtype=FQDtype.Bin, f_dtype=FQDtype.Bin
            )

        with g.name_scope("fc8"):
            fc8 = fc(fc7, output_channels=10, w_dtype=FQDtype.Bin, f_dtype=None)
    return g
...
def get_cifar10_qnn():
    ...
    with g.as_default():
        ...
        with g.name_scope("fc7"):
            fc7 = fc(
                fc6, output_channels=1024, w_dtype=FQDtype.FXP2, f_dtype=FQDtype.FXP2
            )

        with g.name_scope("fc8"):
            fc8 = fc(fc7, output_channels=10, w_dtype=FQDtype.FXP2, f_dtype=None)
    return g
...
def get_lenet_5_twn():
    ...
    with g.as_default():
        ...
        with g.name_scope("pool0"):
            pool0 = maxPool(
                conv0, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("fc2"):
            fc2 = fc(
                fc1, output_channels=10, w_dtype=FQDtype.FXP2, f_dtype=FQDtype.FXP2
            )
...
def get_vgg_7_twn():
    ...
    with g.as_default():
        ...
        with g.name_scope("pool2_a"):
            pool2_a = maxPool(
                conv2_a, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("pool2_b"):
            pool2_b = maxPool(
                conv2_b, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("pool3_a"):
            pool3_a = maxPool(
                conv3_a, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
        ...
        with g.name_scope("pool3_b"):
            pool3_b = maxPool(
                conv3_b, pooling_kernel=(1, 2, 2, 1), stride=(1, 2, 2, 1), pad="VALID"
            )
...
if __name__ == "__main__":
    ...
    # parser arguments
    argp.add_argument(
        "-c", "--config_file", dest="config_file", default="conf.ini", type=str
    )
    argp.add_argument(
